-- Table structure for work_disability_evaluate_question_0420_测试旧的数据
-- ----------------------------
DROP TABLE IF EXISTS `work_disability_evaluate_question_0420_测试旧的数据`;
CREATE TABLE `work_disability_evaluate_question_0420_测试旧的数据` (
  ...
  `difficulty_level` tinyint(1) DEFAULT NULL COMMENT '试题难易程度：1高级 2中级 3低级',
  ...
  UNIQUE KEY `uk_wdeq_code` (`question_code`) USING BTREE,
  KEY `idx_wdeq_filter` (`is_delete`,`category_l1`,`category_l2`,`category_l3`,`disability_type`,`question_type`) USING BTREE,
  KEY `idx_wdeq_source` (`source_doc`,`version_tag`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=963 DEFAULT CHARSET=utf8mb4 COMMENT='残疾人职业能力测评题库';
