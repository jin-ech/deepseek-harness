-- Table structure for guohua_payment_confirm_bill_detail
-- ----------------------------
DROP TABLE IF EXISTS `guohua_payment_confirm_bill_detail`;
CREATE TABLE `guohua_payment_confirm_bill_detail` (
  ...
  `is_confirm` tinyint(4) DEFAULT 0 COMMENT '是否已回 1是 0否',
  PRIMARY KEY (`id`),
  KEY `idx_payment_confirm_id` (`payment_confirm_id`),
  KEY `idx_bill_detail_id` (`bill_detail_id`),
  KEY `idx_bill_id` (`bill_id`)
) ENGINE=InnoDB AUTO_INCREMENT=23386 DEFAULT CHARSET=utf8mb4 COMMENT='回款认领-开票明细关联表';
