-- Table structure for guohua_regulation_bak
-- ----------------------------
DROP TABLE IF EXISTS `guohua_regulation_bak`;
CREATE TABLE `guohua_regulation_bak` (
  `regulation_id` int(11) NOT NULL AUTO_INCREMENT COMMENT '规章制度id',
  `system_id` varchar(64) DEFAULT NULL COMMENT '所属系统id',
  `regulation_type` tinyint(4) DEFAULT 1 COMMENT '规章制度类型 1一线销售  2销售总监  3区域负责人 4cmo及以上 5业务员',
  `visible_roles` varchar(255) DEFAULT NULL COMMENT '可见角色',
...
) ENGINE=InnoDB AUTO_INCREMENT=18 DEFAULT CHARSET=utf8mb4 COMMENT='国华众联规章制度表';
