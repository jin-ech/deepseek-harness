-- Table structure for market_customer_demand_confirm_record
-- ----------------------------
DROP TABLE IF EXISTS `market_customer_demand_confirm_record`;
CREATE TABLE `market_customer_demand_confirm_record` (
  `confirm_id` bigint(20) NOT NULL COMMENT '主键（应用层 get_unique_num_id）',
  `demand_id` bigint(20) NOT NULL COMMENT '需求 id（guohua_demand.demand_id）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='客户确认需求记录表';
