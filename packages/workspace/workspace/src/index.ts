...
export class WorkspaceRegistry extends Service {
  ...
  /**
   * Remove one session from the registry-global archive set. A session not in
   * the set resolves without writing. Used when a session is deleted so its
   * stale archive entry does not linger.
