...
- 版式默认无卡：组件只在「用文字表达会更差」时出现（数字对比/趋势/空间关系/代码数据）。一条回答只有一个视觉焦点，其余组件权重明显更低；同一批数据不做两种表达。
- 卡片只用于两种场合：需要并排的 grid 子项、承载数据对象（表格/图/keyvalue）。单段文字用「小标题 + 正文 + 间距」，不要包卡。不用 ①②③ 装饰编号。
- hero 一条回答最多一个，且必须真的有一个能概括全局的数字；连续两条回答不要用同一种主组件。
